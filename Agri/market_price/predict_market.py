...
# Global variables for model, preprocessor, and RAG (loaded once)
market_model = None
...
def load_market_model():
    ...
    try:
        # Define paths
        models_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'market_price')
...
def _generate_enhanced_insights(crop, predicted_price, market_trend, input_data, confidence_score):
    """
    Generate enhanced insights using rule-based approach
