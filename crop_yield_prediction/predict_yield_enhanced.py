"""
Enhanced Crop Yield Prediction Script with LLM Integration

This script loads the trained crop yield prediction model and provides
detailed explanations and recommendations using LLM-generated content.
"""
...
def generate_llm_explanation(predicted_yield, input_data, insights):
    """
    Generate a natural language explanation using LLM with RAG for enhanced context.
    ...
    try:
        ...
        - Irrigation Used: {'Yes' if input_data.get('Irrigation_Used', False) else 'No'}
        ...
        - Risk Assessment: {insights.get('risk_assessment', 'N/A')}
        - Confidence Level: {insights.get('confidence_level', 'N/A')}
        - Comparison to Benchmarks: {insights.get('comparison_to_benchmarks', 'N/A')}
        """
        
        # Add key factors to context
        ...
        # Create prompt for LLM with specific instructions
        prompt = f"""
        You are an expert agricultural advisor. Based on the following crop yield prediction analysis, 
        provide a comprehensive, detailed explanation with actionable recommendations for the farmer.
        
        {context}
        
        Please provide:
        1. A detailed explanation of what this yield prediction means for the farmer
        2. Specific factors that are positively or negatively affecting the yield
        3. Detailed, actionable recommendations to optimize yield (at least 5 specific actions)
        4. Potential challenges or risks to be aware of
        5. Best timing for key activities
        6. Resource requirements and cost considerations
        7. Long-term strategies for improving yield potential
        ...
        print(f"Error in LLM explanation generation: {str(e)}")
...
def predict_yield_enhanced(input_data):
    ...
    try:
        ...
        explanation = generate_llm_explanation(predicted_yield, input_data, insights)
